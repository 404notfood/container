...
Object.assign(Templates, {
  ...
  readme(config) {
    ...
    const envDocs = {
      ...
      'vps-hestia': { name: 'VPS + HestiaCP', desc: 'Le SSL et le reverse proxy sont gérés par HestiaCP. Les containers tournent en backend et HestiaCP fait le proxy Nginx vers eux.', scriptType: 'bash', url: `https://${config.domain || 'example.com'}`, note: 'SSL géré automatiquement par HestiaCP (Let\'s Encrypt)', extraSetup: `\n### Installation HestiaCP\n\n1. Copiez le dossier du projet sur votre VPS\n2. Installez les templates Nginx proxy :\n\`\`\`bash\nsudo bash hestia/install.sh --domain ${config.domain || 'example.com'} --user ${config.hestiaUser || 'admin'}\n\`\`\`\n3. Démarrez les containers :\n\`\`\`bash\nchmod +x start-containers.sh\n./start-containers.sh\n\`\`\`\n\nLe template proxy redirige le trafic de HestiaCP vers vos containers sur le port ${config.hestiaProxyPort || '8080'}.\n` }
    };
